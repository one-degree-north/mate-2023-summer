...
def main(stop_event=None, use_stop_event=False, debug=False):
    ...
    bno_data_lock = threading.Lock()
    thrust_controller = ThrusterController(bno_data=bno_data, move_delta_time=config['thruster_move_delta'], stop_event=stop_event, use_stop_event=use_stop_event, debug=not not config['debug'], passthrough=True, data_lock=bno_data_lock)
    server = OPiServer(server_address=(addr, config['udp_port']), stop_event=stop_event, use_stop_event=use_stop_event)
    interface = MCUInterface(serial_port=serial_port, stop_event=stop_event, use_stop_event=use_stop_event, bno_data=bno_data, data_lock=bno_data_lock)
    ...
    return thrust_controller, server, interface
