...
void cmd_set_servos(u8 param, u8 len, u8 *data) {
    if (param != 0x2F) return;
    if (len != 4) return;

    u16 val1 = data[0] * 0x100 + data[1];
    u16 val2 = data[2] * 0x100 + data[3];

    servo_pos[0] = val1;
    servo_pos[1] = val2;
}

void cmd_get_servos(u8 param, u8 len, u8 *data) {
    return cmd_return_servos();
}
...
void cmd_return_servos() {
    u8* t0 = (u8*) &servo_pos[0];
    u8* t1 = (u8*) &servo_pos[1];
...
void uart_parse_packet() {
    ...
    switch (cmd) {
        ...
        case 0x1A:
            ...
            break;
        case 0x28:
            cmd_set_servos(param, len, data);
            break;
        case 0x2A:
            cmd_get_servos(param, len, data);
            break;
        case 0x32:
...
void servo_init_pwm_50hz(u8 pin) {
    ...
    // servo runs at 50Hz, supposedly with pulse widths between 500us to 6000us
    // 96MHz core divided by 192 results in 500kHz (2us), multiplied by 9999+1 = 20ms.
    // loop will be kinda weird:
    // 500us = 250 / 10000
    // 660us = 330 / 10000
    // 1000us = 500 / 10000
    // 1500us = 750 / 10000
    ...
    // 4720us = 2360 / 10000
    // 5410us = 2705 / 10000
    pwm_config_set_clkdiv_int(&config, 192);
    pwm_init(slice, &config, true);
    pwm_set_wrap(slice, 9999);
    pwm_set_chan_level(slice, channel, 750); // set to 1500us
}

void servo_output(u8 servo, u16 level) {
    // level: [0, 3000], maps to [0us, 6000us]
    if (!assertRange(level, 0, 3000))
        return;

    u8 slice = pwm_gpio_to_slice_num(servo_pins[servo]);
    u8 channel = pwm_gpio_to_channel(servo_pins[servo]);
...
void setup_outputs() {
    ...
    for (int i = 0; i < 2; ++i) {
        servo_init_pwm_50hz(servo_pins[i]);
        servo_output(i, 750);
    }
...
void loop_outputs() {
    ...
    // u16 max_delta = MIN(MAX_DELTA_POS * elapsed_us / 1000000000, 0xFFFF);
    // old max_delta calculation
...
int main() {
    bi_decl(bi_program_description("SAS MATE 2023 R&D Firmware V4"));
